...
namespace Websocket.Api.Extensions
{
    public static class AppBuilderExtensions
    {
            ...
            if (webHandlerService is not null)
            {
                app.Use(async (context, next) =>
                {
                    if (context.Request.Path == "/ws")
                    {
                        ...
                        {
                            await webHandlerService.ClientConnected(context);
        ...
        public static void UseServerHandler(this IApplicationBuilder app, IConfiguration configuration, ILogger logger)
        {
            logger.LogMessage($"Starting Server handler service.");

            var serverHandlerService = app.ApplicationServices.GetService<IServerHandlerService>();
